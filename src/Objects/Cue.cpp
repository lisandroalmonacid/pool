...
void Cue::draw() {
    if (!scene->ballsMoving()) {
        ...
        if (!scene->ballInHandMode) {
            ...
            double x = ((cueW + 2*ballRadius + cueBallSpace + scene->shotForce*4) * cosf(cueAngle) + cueW)/2;
            double y = ((cueW + 2*ballRadius + cueBallSpace + scene->shotForce*4) * sinf(cueAngle) + cueH)/2;
